...
####### Run ORCA Shiny Application (Recommended: via Docker)

# This function launches the ORCA Shiny application inside a Docker container.  
# It is the **recommended method for running ORCA on servers or remote machines**, 
# as it avoids the need to manually install dependencies and ensures environment reproducibility.
#
# Alternatively, the application can be run locally by installing the package from GitHub.
#
###  How to Run ORCA
#### Option 1: Docker (Recommended for Servers)
# Using the provided fuction in this scritpt.

# docker.application.run()
#
# This will:
# - Pull the latest Docker image of ORCA if not already present
# - Launch the Shiny application at `http://localhost:3838`
# - Allow you to type `'s'` in the console to stop the application
#
# ---
#
...
# ORCA::runApp()  # or your preferred run function


# NOTE: the server has an old version of R so: Let's use docker!!
