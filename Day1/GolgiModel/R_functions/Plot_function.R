...
ModelAnalysisPlot <-function(tracefile){
  ...
  #                    "Sacm1lTot","MAPK13_PKD","Pi4kbxTot","PtdinsTrash","Fn1")
  ...
  #                    "Itga5Itgb1Fn1nMa",
  #                    "Itga5Itgb1Fn1nMbR",
  #                    "Vesicle_FN_Itga5itgb1","MAPK13_PKD")
  ...
  trace[,"Pi4kbxTot"]  = trace[,"Pi4kbX"] + trace[,"Pi4kbx"]
  trace[,"PKDxTot"]  = trace[,"PrkdX"] + trace[,"PKDx"] + trace[,"MAPK13_PKD"]
  trace[,"PtdInsTot"]  = trace[,"PtdIns4Pb"] + trace[,"PtdIns"]
  trace[,"Sacm1lTot"]  = trace[,"Sacm1lX"] + trace[,"Sacm1lx"]
  trace[,"Itga5Itgb1Fn1nMTot"]  = trace[,"Itga5Itgb1Fn1nMb"] + trace[,"Itga5Itgb1Fn1nMbR"]
  trace[,"AtacTot"]  = trace[,"Atac"] + trace[,"Ppfia1_Atacn"]
  trace[,"AtacmTot"]  = trace[,"Atacm"] + trace[,"Ppfia1_Atacm"]
  ...
  v = traces[traces$Time == 0 & traces$Place =="PtdIns4Pb","V"] * (Ref.DF[Ref.DF$Place == "PtdIns4Pb","V"])
  Ref.DF[Ref.DF$Place == "PtdIns4Pb", "V"] <- as.numeric(v)
  ...
  # d = data.frame(Place = c("Atac","Atacm"), V = NaN )
  # Ref.DF <- rbind(Ref.DF,d)
  ATAC = traces[traces$Time == 0 & traces$Place == "AtacTot","V"] + traces[traces$Time == 0 & traces$Place == "AtacmTot","V"]
  Ref.DF[Ref.DF$Place == "AtacTot", "V"] = ATAC * Ref.DF[Ref.DF$Place == "AtacTot", "V"]
  Ref.DF[Ref.DF$Place == "AtacmTot", "V"] = ATAC * Ref.DF[Ref.DF$Place == "AtacmTot", "V"]
    
  # d <- unique(traces[traces$Place %in% Ref.DF$Place,"Place"])
  # 
  # d = data.frame(Place = d, V = NaN )
  # Ref.DF <- rbind(Ref.DF,d)
  Ref.DF[Ref.DF$Place == "Fn1", "V"] <- NA
  Ref.DF$Time = max(traces$Time)
  
  ### Green points
  BlueDF = data.frame(Time = 24,
                      Place = c("Mapk13","Atac","PtdIns4Pb","PrkdX",
                                "Pi4kbxTot","PKDxTot","PtdInsTot","Sacm1lTot","MAPK14","VesicleFnC"),
                      V = c(5.38,4.445,
                            trace[1,"PtdIns4Pb"],trace[1,"PrkdX"],
                            4.39,4.39,32,33.75, 58.18,270)
  )
  ########
  lev =  c(
    "Ppfia1m","Ppfia1n","Ppfia1_Atacm","Ppfia1_Atacn","Atacm","Atac","Mapk13","MAPK14",
    "PKDx","MAPK13_PKD","PrkdX","Pi4kbx", "Pi4kbX","PtdIns","PtdIns4Pb","PtdinsTrash",
    "Sacm1lx","Sacm1lX",
    "VesicleFnC",
    ...
    "PKDxTot","Pi4kbxTot","PtdInsTot","Sacm1lTot","Itga5Itgb1Fn1nMTot",
    "AtacTot","AtacmTot"
  )
  lab = c("PPFIA1m","PPFIA1n","PPFIA1_ATACm","PPFIA1_ATACn","ATACm","ATACn","MAPK13",
          "MAPK14","PKD","Inactive_PKD","PKD*","PI4KB","PI4KB*","PI" ,"PI4P","PI_Buffer",
          "SACM1L","SACM1L*",
          ...
          "Sum_PKD", "Sum_PI4KB","Sum_PI","Sum_SACM1L", "Sum_Integrins_Mb", "Sum_ATACn", "Sum_ATACm")

  traces$Area = NA
  traces$Area[traces$Place %in% c("Mapk13","MAPK13_PKD","MAPK14","MAPK13_PKD","Pi4kbxTot", "PKDxTot","Pi4kbxTot","PtdInsTot","Sacm1lTot",
                                  "PKDx","PrkdX","Pi4kbx", "Pi4kbX","PtdIns","PtdIns4Pb","PtdinsTrash","Sacm1lx","Sacm1lX","Fn1")] = "Golgi Apparatus"
  traces$Area[traces$Place %in% c("Atac","Ppfia1n","Ppfia1_Atacn","AtacTot")] = "Nucleus"
  traces$Area[traces$Place %in% c("VesicleFnC","Vesicle_Itga5Itgb1_Rab11bC",
                                  "Vesicle_FN_Itga5itgb1",
                                  "Itga5Itgb1Fn1Rab21C",
                                  "Itga5Itgb1G","Lysosomes")] = "Integrin recycling loop"
  traces$Area[traces$Place %in% c("Itga5Itgb1Fn1nMb","Itga5Itgb1Fn1nMbR","Itga5Itgb1Fn1nMTot","AtacmTot","Atacm",
                                  "Ppfia1m","Ppfia1_Atacm")] = "Basal membrane"
...
SensitivityPlot <-function(rank=T,folder = "results_sensitivity_analysis",model,finaltime){
  ...
  #PlaceNotToPlot = c("Ppfia1PtprfAtacm", "Ppfia1Ptprf","Ppfia1PtprfAtacn","Ppfia1Ptprfn", "Sacm1l")
  ...
  ##### Modifica per plottare i reference con percentuali o somme in maniera corretta:
  v = traces[traces$Time == 0 & traces$Place =="PtdIns4Pb","V"] * (1-value["PtdIns4Pb"])
  Ref.DF[Ref.DF$Place == "PtdIns4Pb", "V"] = as.numeric(v)
  
  v = traces[traces$Time == 0 & traces$Place == "Atacm","V"] * (value["Atacm"])
  Ref.DF[Ref.DF$Place == "Atacm", "V"] = unique(as.numeric(v))
  v = traces[traces$Time == 0 & traces$Place == "Atacn","V"] * (1-value["Atacm"])
  Ref.DF[Ref.DF$Place == "Atacn", "V"] = unique(as.numeric(v))
  ...
  for(x in 1:length(config) ){
    if(!config[[x]][[bestConfig]][[1]] %in% TrNo )
    {
      ...
      init = config[[x]][[bestConfig]][[3]][c("PtdIns4Pa","Prkd1X","PtdIns")]
    }
  }
  optim = c(optim,init)
